...
public class TarifaIvaListFxController extends AnchorPane {
	...
	ObservableList<TarifaIva> tarifasIvasList;
	...
	private void initialize() {
		tarifasIvasList = FXCollections.observableList(tarifaIvaController.tarifaIvaList());
		tblLista.setItems(tarifasIvasList);		
		idCol.setMinWidth(0);
		idCol.setMaxWidth(0);
		...
		Pagination paginacion=new Pagination((tarifasIvasList.size()/ rowsPerPage() + 1),0);
		paginacion.setPageFactory(new Callback<Integer, Node>() {
            @Override
            public Node call(Integer pageIndex) {
                if (pageIndex > tarifasIvasList.size() / rowsPerPage() + 1) {
                    return null;
                } else {
	...
	private Node createPage(int pageIndex) {
	    ...
	    int toIndex = Math.min(fromIndex + rowsPerPage(), tarifasIvasList.size());
	    tblLista.setItems(FXCollections.observableArrayList(tarifasIvasList.subList(fromIndex, toIndex)));

	    return new BorderPane((Node) tarifasIvasList);
	}
	...
	private void cargarObjetoSeleccionadaEnFormulario() {
		...
		if (tarifaIva != null) {
			posicionObjetoEnTabla = tarifasIvasList.indexOf(tarifaIva);
	...
	private void actualizar() {
		...
		if (error == null) {
			tarifasIvasList.set(posicionObjetoEnTabla, tarifaIva);
	...
	private void eliminar() {
		...
		if (error == null) {
			tarifasIvasList.remove(getObjetoSeleccionadoDeTabla());
	...
	private void btnBuscarClick(ActionEvent event) {
		...
		if (tarifaIvaList != null) {
			tarifasIvasList = FXCollections.observableList(tarifaIvaList);
			tblLista.setItems(tarifasIvasList);
		} else {
			tarifasIvasList.clear();
		}
		btnNuevoClick(null);
